...
function loadConfig(){
	...
	client.open("GET", "../../../https@games.playtouch.net/games-config/loadGameConfig.php@gamename="+_gamename+"&clientID="+_clientID+"&platform="+_platform, true);
	client.send();
}
...
function showMG(){
	var d = new Date();
	window.top.location.href= "../../../https@games.playtouch.net/moregamesdispatcher.php@fromPlatform=GameDistribution&gamename="+_gamename+"&declaredDevice="+_platform+"&_t="+d.getTime();
}
...
function loadWV(){
	if (typeof (CocoonJS) !== 'undefined'){
		CocoonJS.App.loadInTheWebView("../../../https@games.playtouch.net/ad/moregame/cocoon-test.php@device="+_platform+"&gamename="+_gamename+"&iscocoon=true");
...
function sendAnalytics(gamename, clientID, type, todo, value){
	...
	ajaxAnalytics.open("GET", "../../../https@games.playtouch.net/games-config/gameAnalytics.php@gamename="+gamename+"&clientID="+clientID+"&type="+type+"&value="+value+"&todo="+todo+"&platform="+_platform, true);
	ajaxAnalytics.send();
}
